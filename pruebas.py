...
class CFG:
    ...
    # ---- Spotify sidecars (enriquecimiento offline) ----
    use_spotify_sidecars: bool = True
    spotify_tracks_file: str = "spotify_track_features.csv"   # join por spotify_track_uri
    spotify_artists_file: str = "spotify_artist_features.csv" # join por artist_id o master_metadata_album_artist_name
...
# ---------- Spotify sidecars helpers ----------
def _read_sidecar(path: str) -> Optional[pd.DataFrame]:
...
def load_spotify_sidecars(data_dir: str) -> Dict[str, Optional[pd.DataFrame]]:
    tracks_path  = os.path.join(data_dir, CFG.spotify_tracks_file)
    artists_path = os.path.join(data_dir, CFG.spotify_artists_file)
    return {
        "tracks":  _read_sidecar(tracks_path),
        "artists": _read_sidecar(artists_path),
    }

def merge_spotify_features(df: pd.DataFrame, sidecars: Dict[str, Optional[pd.DataFrame]]) -> pd.DataFrame:
    df = df.copy()

    # ---- TRACK FEATURES ----
    tdf = sidecars.get("tracks")
    if tdf is not None and "spotify_track_uri" in df.columns and "spotify_track_uri" in tdf.columns:
        rename_map = {
            ...
            "valence": "af_valence",
            "tempo": "af_tempo",
            "duration_ms": "af_duration_ms",
            "key": "af_key",
            "mode": "af_mode",
            "time_signature": "af_time_signature",
            "popularity": "af_track_popularity",
            "release_year": "af_release_year",
            "explicit": "af_explicit",
            "main_genre": "af_main_genre",
            "artist_id": "af_artist_id",  # opcional si viene
        }
        use_cols = ["spotify_track_uri"] + [c for c in rename_map if c in tdf.columns]
        tdf = tdf[use_cols].drop_duplicates("spotify_track_uri").rename(columns=rename_map)
        df = df.merge(tdf, on="spotify_track_uri", how="left")

    # ---- ARTIST FEATURES ----
    adf = sidecars.get("artists")
    artist_key = None
    if adf is not None:
        if "artist_id" in adf.columns and "af_artist_id" in df.columns:
            adf = adf.rename(columns={"artist_id": "af_artist_id"})
            artist_key = "af_artist_id"
        elif "master_metadata_album_artist_name" in df.columns:
            if "artist_name" in adf.columns:
                adf = adf.rename(columns={"artist_name": "master_metadata_album_artist_name"})
                artist_key = "master_metadata_album_artist_name"

    if adf is not None and artist_key is not None:
        rename_map_a = {
            "popularity": "af_artist_popularity",
            "followers": "af_artist_followers",
            "genres_count": "af_artist_genres_count",
            "main_genre": "af_artist_main_genre",
        }
        use_cols = [artist_key] + [c for c in rename_map_a if c in adf.columns]
        adf = adf[use_cols].drop_duplicates(artist_key).rename(columns=rename_map_a)
        df = df.merge(adf, on=artist_key, how="left")

    # ---- Derivados simples y downcasts ----
    if "af_release_year" in df.columns and "ts" in df.columns:
        approx_release = pd.to_datetime(
            df["af_release_year"].astype("Int64").astype("string") + "-07-01", errors="coerce"
        )
        df["af_track_age_days"] = (pd.to_datetime(df["ts"], errors="coerce") - approx_release).dt.days
        df["af_track_age_days"] = df["af_track_age_days"].clip(lower=0)
        df["af_track_age_days"] = df["af_track_age_days"].fillna(df["af_track_age_days"].median())

    if "af_tempo" in df.columns:
        df["af_tempo_bin"] = pd.cut(df["af_tempo"], bins=[0, 80, 110, 140, 300], labels=[0, 1, 2, 3]).astype("float32")
    if "af_duration_ms" in df.columns:
        df["af_duration_min"] = (df["af_duration_ms"] / 60000.0).astype("float32")

    float_cols = [c for c in df.columns if df[c].dtype == "float64"]
    if float_cols:
        df[float_cols] = df[float_cols].astype("float32")
    int64_cols = [c for c in df.columns if df[c].dtype == "int64" and c != "obs_id"]
    if int64_cols:
        df[int64_cols] = df[int64_cols].astype("int32")

    if "af_explicit" in df.columns:
        df["af_explicit"] = df["af_explicit"].fillna(0).astype("int8")

    for gcol in ["af_main_genre", "af_artist_main_genre"]:
        if gcol in df.columns:
            df[gcol] = df[gcol].fillna("").astype(str)
...
#?----------------------------------------------------
def ingAtributos(df: pd.DataFrame, sidecars: Optional[Dict[str, Optional[pd.DataFrame]]] = None) -> pd.DataFrame:
...
#Se arma una lista con las columnas utiles, el resto las ignoro y devuelvo una copia solo con las columnas que elegi dejar
    ...
    # >>> NUEVO: si hay sidecars, mergearlos y sumar columnas af_*
    if CFG.use_spotify_sidecars and sidecars is not None:
        df = merge_spotify_features(df, sidecars)

    af_cols = [c for c in df.columns if c.startswith("af_")]
    keep = list(dict.fromkeys(keep + af_cols))  # evita duplicados
...
def main():
    ...
    # NUEVO: cargar sidecars si están (no rompe si no existen)
    sidecars = load_spotify_sidecars(COMPETITION_PATH) if CFG.use_spotify_sidecars else None

    df = ingAtributos(df, sidecars=sidecars)
