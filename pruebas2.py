...
# === Helpers NUEVOS (para features) ===

# --- helper nuevo (poner cerca de otros helpers) ---
def _safe_log1p_seconds(s: pd.Series) -> pd.Series:
    # log1p de segundos; asegura no-negativo y maneja NaN
    return np.log1p(s.fillna(0).clip(lower=0)).astype("float32")

def _safe_log1p(s: pd.Series) -> pd.Series:
    out = np.log1p(pd.to_numeric(s, errors="coerce").clip(lower=0))
    return out.replace([np.inf, -np.inf], 0).fillna(0).astype("float32")

def _ema_shifted_by_user(df: pd.DataFrame, value_col: str, user_col: str, alpha: float) -> pd.Series:
    """
    EMA no-leaky por usuario: usa shift(1) antes de acumular.
    Requiere que el df esté ordenado por (user, ts) antes de llamar.
    """
    v = df[value_col].where((df["is_test"] == 0) & df[value_col].isin([0,1]), np.nan).astype("float32")
    v_shift = v.groupby(df[user_col], observed=True).shift(1)
    global_mean = float(df.loc[(df["is_test"] == 0) & df[value_col].isin([0,1]), value_col].mean())
    v_shift = v_shift.fillna(global_mean)

    ema = []
    last = {}
    for u, x in zip(df[user_col].values, v_shift.values):
        prev = last.get(u, global_mean)
        cur = alpha * x + (1 - alpha) * prev
        last[u] = cur
        ema.append(cur)
    return pd.Series(ema, index=df.index, dtype="float32")
...
# Merge opcional de audio features de Spotify (si existe archivo CSV)
def maybe_merge_audio_features(df: pd.DataFrame, data_dir: str) -> pd.DataFrame:
    """
    Busca 'spotify_audio_features.csv' en data_dir y mergea por spotify_track_uri.
    Si no existe, devuelve df igual. No pisa columnas existentes.
    """
    path = os.path.join(data_dir, "spotify_audio_features.csv")
    if not os.path.exists(path):
        return df

    af = pd.read_csv(path)
    if "spotify_track_uri" not in af.columns:
        return df

    # columnas esperadas típicas
    cols = ["spotify_track_uri","danceability","energy","valence","acousticness",
            "instrumentalness","liveness","speechiness","tempo","loudness","key","mode","time_signature"]
    keep = [c for c in cols if c in af.columns]
    af = af[keep].drop_duplicates("spotify_track_uri")

    # downcast a float32 / int16
    for c in af.columns:
        if c == "spotify_track_uri":
            continue
        if str(af[c].dtype).startswith(("float","int")):
            if c in ["key","mode","time_signature"]:
                af[c] = pd.to_numeric(af[c], errors="coerce").fillna(0).astype("int16")
            else:
                af[c] = pd.to_numeric(af[c], errors="coerce").astype("float32")

    df = df.merge(af, on="spotify_track_uri", how="left")
    return df
...
#?----------------------------------------------------
#? 3) INGENIERIA DE ATRIBUTOS BASICA  (MEJORADA)
#?----------------------------------------------------
def ingAtributos(df: pd.DataFrame) -> pd.DataFrame:
    # Orden estable por usuario/tiempo
    ...
    # Popularidad de track (solo usando TRAIN para evitar leakage)
    ...
    # Actividad del usuario (solo TRAIN)
    ...
    # Δt previo y siguiente por usuario
    if {"username", "ts"}.issubset(df.columns):
        df["user_dt_prev"] = (
    ...
    # Media acumulada no-leaky de skip por usuario
    if {"username", "target", "is_test"}.issubset(df.columns):
        ...
        prev_sum = (df.groupby("username", observed=True)["target_trn"].cumsum() - df["target_trn"]).astype("float32")
        prev_cnt = (df.groupby("username", observed=True)["is_trn"].cumsum() - df["is_trn"]).astype("float32")
    ...
    if {"spotify_track_uri", "target", "is_test"}.issubset(df.columns):
        ...
        for tr_idx, va_idx in kf.split(idx_trn):
            ...
            means = df.loc[tr_ids].groupby("spotify_track_uri")["target"].mean()
            df.loc[va_ids, "track_skip_ratio"] = df.loc[va_ids, "spotify_track_uri"].map(means)
        global_mean = float(df.loc[mask_trn, "target"].mean())
        df["track_skip_ratio"] = df["track_skip_ratio"].fillna(global_mean).astype("float32")
    else:
        df["track_skip_ratio"] = 0.0

    # Target encoding OOF (suavizado) para columnas seleccionadas
    ...
    # Identificador/posición/longitud de sesión
    ...
    if {"username", "target"}.issubset(df.columns):
        df["prev_target"] = df.groupby("username", observed=True)["target"].shift(1).fillna(0).astype("int8")

        prev = df.groupby("username", observed=True)["target"].shift(1)
        df["user_skip_roll5"] = (
            prev.groupby(df["username"], observed=True)
    ...
    # === NUEVO: recencia por artista y por track (sin length mismatch) ===
    if {"username", "master_metadata_album_artist_name", "ts"}.issubset(df.columns):
        ukey = df["username"].astype(str).fillna("__NA__")
        akey = df["master_metadata_album_artist_name"].astype(str).fillna("__NA__")
        last_time_artist = df.groupby([ukey, akey], observed=True)["ts"].shift(1)
        df["user_artist_recency"] = _safe_log1p_seconds((df["ts"] - last_time_artist).dt.total_seconds())
    else:
        df["user_artist_recency"] = 0.0

    if {"username", "spotify_track_uri", "ts"}.issubset(df.columns):
        ukey = df["username"].astype(str).fillna("__NA__")
        tkey = df["spotify_track_uri"].astype(str).fillna("__NA__")
        last_time_track = df.groupby([ukey, tkey], observed=True)["ts"].shift(1)
        df["user_track_recency"] = _safe_log1p_seconds((df["ts"] - last_time_track).dt.total_seconds())
    else:
        df["user_track_recency"] = 0.0
    ...
    keep = [
        ...
        # nuevas
        "user_artist_recency", "user_track_recency",
    ]
...
def encodeCategoricas(df: pd.DataFrame):
    ...
    # OJO: sacamos username de las features (se usa como grupo, no como feature)
...
def main():
    ...
    # MERGE opcional con audio features (si existe spotify_audio_features.csv)
    df = maybe_merge_audio_features(df, COMPETITION_PATH)
